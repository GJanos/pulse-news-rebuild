...
| `src/screens/SplashScreen.tsx`               | Animated loading screen (Reanimated v4 pulse dots); shown during `auth-check` / `prefs-loading`      |
| `src/screens/LoginScreen.tsx`                | Sign-in / sign-up toggle; zxcvbn strength meter; `onSignIn`, `onSignUp`, `onResetPassword`           |
| `src/screens/ResetPasswordScreen.tsx`        | Password update form; strength meter; shown when `isPasswordRecovery` is true                        |
...
| `src/hooks/useSupabaseAuth.ts`               | Session management — sign in/up/out/reset/delete; exports `AuthActions` + `SignUpResult`             |
| `src/hooks/useDeepLinkRecovery.ts`           | `pulse://reset-password` PKCE + implicit token exchange; dedup via `useRef`                          |
| `src/hooks/useAuthInit.ts`                   | Subscribes to Supabase auth events; drives boot state machine to `unauthenticated` / `prefs-loading` |
